...
const Popup = () => {
  ...
  return (
    <div className={`App absolute inset-0 flex flex-col overflow-hidden ${isLight ? 'bg-amber-50' : 'bg-gray-800'}`}>
      <div className="flex justify-between items-center px-4 py-2 bg-yellow-100 border-b border-[#ffdd79] h-8">
        ...
        {popupSettings.userName && (
          <div className="flex items-center text-sm">
            <span
              className={`mr-1 shrink-0 grow-0 inline-block w-18 ${!popupSettings.isMultiUser ? 'font-bold' : ''} text-yellow-900`}>
              My Notes
            </span>
            ...
            <span
              className={`ml-1 shrink-0 grow-0 w-18 ${popupSettings.isMultiUser ? 'font-bold' : ''} text-yellow-900`}>
              All Notes
            </span>
          </div>
        )}
      ...
      {popupSettings.userName && popupSettings.isMultiUser && (
      ...
      {!popupSettings.userName && (
        <input
          type="text"
          className="mx-2 my-2 px-2 py-2 h-8 bg-yellow-200 rounded-md text-xs text-yellow-900"
          placeholder="Type your name and press enter to start"
          onKeyUp={e => {
            if (e.key === 'Enter') {
              popupSettingsStorage.setUserName(e.currentTarget.value);
            }
          }}
        />
      )}
